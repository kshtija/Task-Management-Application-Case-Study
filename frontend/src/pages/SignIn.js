...
const{signIn}=useContext(AuthContext); const nav=useNavigate();
return <Container maxWidth='xs'><Box sx={{mt:6}}>
<Typography variant='h5'>Sign In</Typography>
<form onSubmit={async ev=>{ev.preventDefault(); try{await signIn(u,p); nav('/');}catch(x){ce('Failed');}}}>
...
{e&&<Typography color='error'>{e}</Typography>}<Button type='submit'variant='contained'fullWidth sx={{mt:2}}>Sign In</Button>
</form></Box></Container>;};
